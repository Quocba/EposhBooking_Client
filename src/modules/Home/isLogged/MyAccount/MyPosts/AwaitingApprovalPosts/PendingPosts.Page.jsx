...
import { filterPostsPending } from "../../../../../../utils/filter";
...
const PendingPostsPage = () => {
  ...
  const listPending = filterPostsPending(data);

  const postsPerPage = 6;
  const indexOfLastPost = currentPage * postsPerPage;
  const indexOfFirstPost = indexOfLastPost - postsPerPage;
  const currentPost = listPending?.slice(indexOfFirstPost, indexOfLastPost);
  const totalPagePost = Math.ceil(listPending?.length / postsPerPage);
            ...
            >
              <Typography
                sx={{
                  position: "absolute",
                  top: 10,
                  bgcolor: themeColors.status_Secondary,
                  p: "5px 10px",
