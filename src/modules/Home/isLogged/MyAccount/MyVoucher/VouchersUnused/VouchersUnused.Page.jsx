...
import { filterVoucherUnused } from "../../../../../../utils/filter";

const VouchersUnusedPage = () => {
  const dispatch = useDispatch();

  const [data, setData] = useState([]);
  const [fetchData, setFetchData] = useState(false);

  let listVoucherUnused = filterVoucherUnused(data);
  ...
  const currentVoucher = listVoucherUnused?.slice(
    indexOfFirstVoucher,
    indexOfLastVoucher
  );
  const totalPageVoucher = Math.ceil(
    listVoucherUnused?.length / vouchersPerPage
  );
...
export default VouchersUnusedPage;
