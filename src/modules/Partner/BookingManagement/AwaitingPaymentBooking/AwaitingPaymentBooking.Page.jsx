...
import { filterBookingPayment } from "../../../../utils/filter";
...
const AwaitingPaymentBookingPage = () => {
  ...
  const listAwaitingPayment = filterBookingPayment(data);

  const bookingsPerPage = 10;
  const indexOfLastBooking = currentPage * bookingsPerPage;
  const indexOfFirstBooking = indexOfLastBooking - bookingsPerPage;
  const currentBooking = listAwaitingPayment?.slice(
    indexOfFirstBooking,
    indexOfLastBooking
  );
  const totalPageBooking = Math.ceil(
    listAwaitingPayment?.length / bookingsPerPage
  );
  ...
  return (
    <>
      ...
      >
        {listAwaitingPayment?.length <= 1
          ? `Total Booking: ${listAwaitingPayment?.length}`
          : `Total Bookings: ${listAwaitingPayment?.length}`}
      </Typography>
...
export default AwaitingPaymentBookingPage;
