...
const CanceledBookingPage = () => {
  ...
  const listCanceled = filterBookingCanceled(data);

  const bookingsPerPage = 10;
  const indexOfLastBooking = currentPage * bookingsPerPage;
  const indexOfFirstBooking = indexOfLastBooking - bookingsPerPage;
  const currentBooking = listCanceled?.slice(
    indexOfFirstBooking,
    indexOfLastBooking
  );
  const totalPageBooking = Math.ceil(listCanceled?.length / bookingsPerPage);
  ...
  return (
    <>
      ...
      >
        {listCanceled?.length <= 1
          ? `Total Booking: ${listCanceled?.length}`
          : `Total Bookings: ${listCanceled?.length}`}
      </Typography>
...
export default CanceledBookingPage;
