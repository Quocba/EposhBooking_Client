...
import SpecialPriceModalUpdate from "./SpecialPrice.ModalUpdate";
...
const InformationModalUpdate = ({ children, data }) => {
  let listImagesUrl = [];
  let listImagesFile = [];
  ...
  const [selectedFiles, setSelectedFiles] = useState(
    (data?.listImages || []).map((image) => ({
      img: `${URL_IMAGE}${image?.image}`,
      imageID: image?.imageID,
      isNew: false,
    }))
  );
  const [typeOfRoom, setTypeOfRoom] = useState(data?.typeRoom || "");
  const [numberCapacity, setNumberCapacity] = useState(
    data?.numberCapacity || ""
  );
  const [quantity, setQuantity] = useState(data?.quantity || "");
  const [sizeRoom, setSizeRoom] = useState(data?.size || "");
  const [numberBed, setNumberBed] = useState(data?.numberBed || "");
  const [typeBed, setTypeBed] = useState(data?.typeOfBed || "");
  ...
  const getNewListImages = () => {
    selectedFiles.forEach((file) => {
      if (file?.isNew) {
        listImagesFile.push(file?.file);
        return listImagesFile;
      } else {
        listImagesUrl.push(file?.img?.split(`${URL_IMAGE}`)[1]);
  ...
  const checkValid = () => {
    if (listImagesUrl.length === 0 && listImagesFile.length === 0) {
      setShowNextModal(false);
    ...
    <>
      ...
      >
              ...
              >
                ...
                >
                  {selectedFiles.map((file, index) => (
                    <Box
                      key={file.isNew ? file.img : file.imageID}
                      position="relative"
                      sx={{
        ...
        <DialogActions sx={{ justifyContent: "space-around" }}>
          ...
          {showNextModal ? (
            <SpecialPriceModalUpdate
              data={{
                id: id,
                imagesURL: listImagesUrl,
                imagesFile: listImagesFile,
                typeRoom: typeOfRoom,
...
export default InformationModalUpdate;
